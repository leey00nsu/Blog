...
react-query를 검색해면 다음과 같은 라이브러리라고 합니다.
...
설명에 있듯이 react 어플리케이션에서 비동기 데이터를 관리하고 캐시하며 상태를 관리하며 갱신할 수 있습니다.

이번에`artfolio` 프로젝트를 진행하면서 , 기존에 `useEffect`를 사용하여 비동기를 처리 하던 방식에서 `react-query` 를 사용하여 관리하는 방식을 채택하였습니다.

기존의 `useEffect`를 이용한 비동기 처리 코드를 보면 다음과 같습니다.
...
위의 22350원과 , 밑의 22350은 현재 금액을 나타내는데 , 이는 웹소켓 연동으로 인하여 서버 데이터와 실시간 동기화 되고 있습니다.

따라서 입찰하기를 누르는 순간 , 실시간 경매가가 갱신되며 두 컴포넌트는 같은 값을 나타내야 합니다.
...
이 문제는 `쿼리키` 를 사용해서 , 각각 요청을 하지만 같은 쿼리키를 사용함으로써 한 쪽이 업데이트되면 다른 한 쪽은 캐싱되어 있는 데이터를 가져옴으로써 해결할 수 있었습니다.

과정을 간단하게 풀어낸다면 다음과 같습니다.

:::info
경매 페이지 데이터 페칭 → 캐싱된 데이터 페칭  
Footer 데이터 페칭 → 입찰하기 요청 → 데이터 변경과 동시에 useQuery 재요청
:::

### 결론

이 외에도 비동기 데이터들을 가져올 때 대부분 react-query를 사용하였는데

isLoading,isError와 같은 변수들로 데이터 상태를 추적할 수 있으며 , 쿼리옵션에 따라 데이터를 자동적으로 페칭하거나 시간을 정할 수 있는 등 편리한 요소가 많으므로 앞으로도 사용할 것 같습니다.
